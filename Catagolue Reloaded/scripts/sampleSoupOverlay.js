...
function overlaySoup(soupURL, soupNumber, totalSoups, apgcode, name) {
	...
	// apgsearch 0.x/1.x (empty string), apgnano 2.x ("n_"), apgmera 3.x
	// ("m_"), apgluxe 4.x ("l_") or hs 1.0 ("h_"), but we have no use for 
	// this at the moment.
	...
	sampleSoupRequest.addEventListener("load", function() {
			...
			// Catagolue does not generate correct sample soup RLE for non-
			// standard symmetries. Such soups are returned as if the symmetry
			// is C1, so we attempt to detect these cases and, if we know how
			// to, fix up these soups.
			//
			...
			if((symmetry != "C1") && (sampleSoup.substr(0, 16) == "x = 16, y = 16, "))
				...
				switch(symmetry) {
					...
					case "1x256X2+1":
						...
						sampleSoup = lines[0].replace("x = 16", "x = " + (16 * linesToCombine * soupBxFactor + soupBxExtra)).replace("y = 16", "y = " + (16 / linesToCombine)) + "\n";
					...
					// the below code doesn't quite work for them. There isn't
					// a whole lot of information on how these soups ARE 
					// generated, though, other than the following statement:
					// 
